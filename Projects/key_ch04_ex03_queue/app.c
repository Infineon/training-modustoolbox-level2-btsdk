 ...
 ******************************************************************/
wiced_thread_t * LED_thread_handle;
...
wiced_result_t app_bt_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:						// Bluetooth Controller and Host Stack Enabled
        ...
        if( WICED_BT_SUCCESS == p_event_data->enabled.status )
        {
			...
			LED_thread_handle = wiced_rtos_create_thread();
			wiced_rtos_init_thread(
					LED_thread_handle,			// Thread handle
					4,                			// Medium Priority
					"LED Thread",					// Name
					LED_thread,					// Function
					1024,						// Stack space for the LED_thread function to use
...
/*******************************************************************************
* Function Name: void LED_thread(uint32_t)
********************************************************************************/
void LED_thread( uint32_t arg )
{
    uint32_t loop;
    uint32_t blinkValue;
